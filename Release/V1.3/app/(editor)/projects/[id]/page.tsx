...
export default function ProjectEditorPage() {
    ...
    for (const file of Array.from(files)) {
      ...
      try {
        ...
        if (type === "video" || type === "audio") {
          // Upload video/audio to TOS for public URL
          ...
          const data = await res.json();
          if (!res.ok || !data.url) {
            toast.error(`${file.name} 上传失败：${data.error || "TOS 上传失败"}`);
            if (data.error?.includes("not configured")) {
              toast.error("请先在「设置」中配置 TOS 对象存储");
            }
            continue;
          }
          url = data.url;
        } else {
          // Upload image locally (can use base64 directly)
          const formData = new FormData();
          formData.append("file", file);
  ...
  function usePrompt(job: any) {
    const p = job.params || {};
    if (p.promptSections) {
      setPromptSections(p.promptSections);
    } else if (p.prompt) {
      setPromptSections((prev) => ({ ...prev, storyboard: p.prompt }));
    }
    if (p.model) setModel(p.model);
    if (p.ratio) setRatio(p.ratio);
    if (p.duration != null) setDuration(String(p.duration));
    if (p.resolution) setResolution(p.resolution);
    if (p.assets?.length) {
      setAssets((prev) => {
        const existing = new Set(prev.map((a) => a.url));
        const newOnes = p.assets.filter((a: any) => !existing.has(a.url));
        return [...prev, ...newOnes];
      });
    }
    toast.success("已加载该结果的提示词和参数");
  }
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="border-b bg-background">
        ...
        >
              ...
              {imageAssets.length > 0 && (
                <div className="flex items-center gap-2">
                  {imageAssets.map((asset) => (
                    <div key={asset.id} className="flex flex-col items-center gap-0.5">
                      <div
                        className={cn(
                      ...
                      <span className="text-[10px] text-muted-foreground">pic</span>
                    </div>
                  ))}
                </div>
              )}
                    ...
                    <div key={asset.id} className="flex flex-col items-center gap-0.5">
                      ...
                      >
                        <video
                          ...
                          muted
                          preload="metadata"
                          className={cn("h-full w-full object-cover transition-all", asset.frozen && "grayscale")}
                        />
                        <button
                          onClick={() => toggleFreeze(asset.id)}
                          className="absolute left-0.5 top-0.5 z-10 flex h-4 w-4 items-center justify-center rounded-full border border-background bg-blue-500 text-white opacity-0 shadow transition-opacity hover:bg-blue-600 group-hover:opacity-100"
          ...
          <div className="flex gap-6">
            {/* Left column */}
            <div className="relative w-2/3 space-y-4">
              {/* Multi-section prompt editor */}
                ...
                <div className="relative aspect-[16/9] bg-muted">
                  {job.status === "completed" && job.resultUrl ? (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 hidden flex-wrap items-center justify-center gap-1.5 bg-black/40 p-1 group-hover:flex">
                        <Button
                          size="icon"
                          variant="secondary"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        className="h-auto px-1.5 py-0.5 text-[10px] text-destructive hover:text-destructive"
      ...
      <Dialog open={!!previewJob} onOpenChange={(open) => !open && setPreviewJob(null)}>
        <DialogContent className="sm:max-w-6xl p-0 overflow-hidden" showCloseButton>
      ...
      <Dialog open={!!detailJob} onOpenChange={(open) => !open && setDetailJob(null)}>
        <DialogContent className="sm:max-w-7xl p-0 overflow-hidden" showCloseButton>
          <DialogTitle className="sr-only">生成结果详情</DialogTitle>
          <DialogDescription className="sr-only">
            {detailJob?.params?.prompt || "生成结果详情"}
          </DialogDescription>
          {detailJob && (
            <div className="flex h-[85vh] max-h-[900px]">
              {/* Left: Video preview (uniform vertical size) */}
              <div className="flex w-[420px] shrink-0 items-center justify-center bg-black">
                {detailJob.resultUrl && isVideoUrl(detailJob.resultUrl) ? (
                  <div className="relative h-full w-full">
                    <video
                      src={detailJob.resultUrl}
                  ...
                  <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
                    ...
                    <span className="mt-2 text-sm">无结果</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-1 flex-col overflow-hidden">
                <div className="flex-1 overflow-y-auto p-6">
                  <h3 className="mb-4 text-lg font-semibold">生成详情</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div className="rounded-lg border bg-muted/50 p-3">
                        ...
                        <div className="font-medium">
                          {modelOptions.find((m) => m.value === detailJob.params?.model)?.label || detailJob.params?.model || "-"}
                        </div>
                      </div>
                      <div className="rounded-lg border bg-muted/50 p-3">
                        <div className="text-xs text-muted-foreground">比例</div>
                        <div className="font-medium">{detailJob.params?.ratio || "-"}</div>
                      </div>
                      <div className="rounded-lg border bg-muted/50 p-3">
                        <div className="text-xs text-muted-foreground">时长</div>
                        <div className="font-medium">{detailJob.params?.duration != null ? `${detailJob.params.duration}s` : "-"}</div>
                      </div>
                      <div className="rounded-lg border bg-muted/50 p-3">
                        <div className="text-xs text-muted-foreground">清晰度</div>
                        <div className="font-medium">{detailJob.params?.resolution || "-"}</div>
                      </div>
                    </div>
                    ...
                    <div className="rounded-lg border bg-muted/50 p-3">
                      ...
                      <div className="whitespace-pre-wrap text-sm leading-relaxed">
                        {detailJob.params?.prompt || "-"}
                      </div>
                    </div>

                    {/* Reference assets */}
                    {detailJob.params?.assets?.length > 0 && (
                      <div className="rounded-lg border bg-muted/50 p-3">
                        <div className="mb-2 text-xs text-muted-foreground">参考素材</div>
                        <div className="flex flex-wrap gap-2">
                          {detailJob.params.assets.map((asset: any, idx: number) => (
                            <div key={idx} className="flex items-center gap-1.5 rounded-md bg-background px-2 py-1 text-xs">
                              {asset.type === "image" && asset.url ? (
                                <div className="relative h-6 w-6 overflow-hidden rounded">
                                  <SafeImage src={asset.url} alt="" fill className="object-cover" />
                                </div>
                              ) : asset.type === "video" ? (
                              ...
                              <span className="max-w-[120px] truncate">{asset.type}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {detailJob.resultUrl && (
                      <div className="rounded-lg border bg-muted/50 p-3">
                        <div className="mb-1 text-xs text-muted-foreground">下载地址</div>
                        <a
                          href={detailJob.resultUrl}
                          target="_blank"
                    ...
                    <div className="rounded-lg border bg-muted/50 p-3">
                      <div className="flex items-center justify-between">
                        <div className="text-xs text-muted-foreground">API 请求报文</div>
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-7 text-xs"
                          onClick={() => setShowApiPayload((v) => !v)}
                        >
                          {showApiPayload ? "隐藏" : "查看"}
                        </Button>
                      </div>
                      {showApiPayload && (
                        <pre className="mt-2 max-h-64 max-w-full overflow-auto whitespace-pre-wrap break-all rounded-md bg-black/80 p-3 text-xs text-green-400">
                  ...
                  {detailJob.resultUrl && (
                    <Button
                      ...
                      onClick={() =>
                        downloadResult(
                          detailJob.resultUrl,
                          `opencinema-${detailJob.id.slice(-6)}.mp4`
                        )
                      }
                    >
